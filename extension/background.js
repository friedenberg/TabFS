...
Routes["/windows"] = {
  ...
  async post(req) {
    return {
      status: 201,
      body: await browser.windows.create({ url: req.body.urls }),
    };
  },
  async get(req) {
...
Routes["/windows/#WINDOW_ID/tabs/#TAB_ID"] = {
  async get({ tabId }) {
    return {
      status: 200,
      body: await browser.tabs.get(tabId),
    };
  },
  async delete({ tabId }) {
  ...
  async readdir() {
    ...
    return {
      entries: [
        ".",
        "..",
        ...infos.map((info) => `${sanitize(info.name)}.${info.id}`),
      ],
    };
  },
};
...
window.fetch(chrome.runtime.getURL("background.js")).then(async (r) => {
  window.__backgroundJS = await r.text();
});
...
// children will be.
for (let i = 10; i >= 0; i--) {
  for (let path of Object.keys(Routes).filter(
    (key) => key.split("/").length === i
  )) {
    path = path.substr(0, path.lastIndexOf("/"));
    if (path == "") path = "/";

    if (!Routes[path]) {
      Routes[path] = makeDefaultRouteForDirectory(path);
    }
  }
    ...
    Routes[key] = {
      async getattr() {
        return {
          st_mode:
            unix.S_IFREG |
            ((Routes[key].read && 0444) | (Routes[key].write && 0222)),
          st_nlink: 1,
          st_size: 100, // FIXME
        };
      },
...
async function onMessage(req) {
  ...
  try {
    const [route, vars] = tryMatchRoute(req);
    const routeFunc = route[req.method.toLowerCase()];

    if (routeFunc == null) {
      response.status = 404;
    } else {
      const routeResponse = await routeFunc({ ...req, ...vars });
      response.status = routeResponse.status;

      if ("body" in routeResponse) {
...
//       return this.store[handle].object;
//     },
...
//       return stringToUtf8Array(stringOrArray);
//     } else {
...
//       await setData(req, utf8ArrayToString(arr));
//       return { size: bufarr.length };
//     },
//     async release({ fh }) {
//       Cache.removeObjectForHandle(fh);
//       return {};
//     },
  ...
  async readdir() {
    ...
    return {
      entries: [
        ...
        "..",
        ...tabs.map(
          (tab) => sanitize(String(tab.title)) + "." + String(tab.id)
        ),
      ],
    };
  },
...
Routes["/tabs/by-window"] = {
  description:
    "Open tabs, organized by window then title; each subfolder represents an open tab.",
  usage: "ls $0",
  getattr() {
    return {
      st_mode: unix.S_IFDIR | 0777, // writable so you can delete tabs
  ...
  async readdir() {
    ...
    return {
      entries: [
        ...
        ...tabs.map(
          (tab) =>
            sanitize(String(tab.windowId) + "." + String(tab.title)) +
            "." +
            String(tab.id)
        ),
      ],
...
// (function () {
//   const routeForTab = (readHandler, writeHandler) =>
...
//     usage: ["cat $0", 'echo "https://www.google.com" > $0'],
//     ...routeForTab(
//       (tab) => tab.url + "\n",
//       (buf) => ({ url: buf })
//     ),
...
(function () {
    ...
    return {
      ...
      debugTab: async function (tabId) {
        ...
        try {
          ...
          if (
            e.message.indexOf("Another debugger is already attached") !== -1
          ) {
      ...
      enableDomainForTab: async function (tabId, domain) {
        ...
        if (domain === "Debugger") {
          TabManager.scriptsForTab[tabId] = {};
        }
  ...
  Routes["/tabs/by-id/#TAB_ID/debugger/resources"] = {
    async readdir({ tabId }) {
      ...
      const { frameTree } = await sendDebuggerCommand(
        tabId,
      ...
      return {
        entries: [
          ".",
          "..",
          ...frameTree.resources.map((r) => sanitize(String(r.url))),
        ],
      };
    },
  };
  // Routes["/tabs/by-id/#TAB_ID/debugger/resources/:SUFFIX"] =
  //   makeRouteWithContents(async ({ path, tabId, suffix }) => {
  //     await TabManager.debugTab(tabId);
  //     await TabManager.enableDomainForTab(tabId, "Page");
  ...
  //         let { base64Encoded, content } = await sendDebuggerCommand(
  //           tabId,
  ...
  Routes["/tabs/by-id/#TAB_ID/debugger/scripts"] = {
    ...
    async readdir({ tabId }) {
      ...
      const scriptFileNames = Object.values(
        TabManager.scriptsForTab[tabId]
  ...
  // Routes["/tabs/by-id/#TAB_ID/debugger/scripts/:FILENAME"] =
  //   makeRouteWithContents(
  //     async ({ tabId, filename }) => {
  //       await TabManager.debugTab(tabId);
  //       await TabManager.enableDomainForTab(tabId, "Page");
  //       await TabManager.enableDomainForTab(tabId, "Debugger");

  //       const { scriptId } = pathScriptInfo(tabId, filename);
  //       const { scriptSource } = await sendDebuggerCommand(
  //         tabId,
  ...
  //       const { scriptId } = pathScriptInfo(tabId, filename);
  //       await sendDebuggerCommand(tabId, "Debugger.setScriptSource", {
  //         scriptId,
  //         scriptSource: buf,
  //       });
  //     }
...
//     if (inputValue === null) {
//       throw new UnixError(unix.ENOENT);
//     } /* FIXME: hack to deal with if inputId isn't valid */
//     return inputValue;
//   },
//   async ({ tabId, inputId }, buf) => {
//     const code = `document.getElementById('${inputId}').value = unescape('${escape(
//       buf
//     )}')`;
...
// (function () {
//   const withWindow = (readHandler, writeHandler) =>
//     makeRouteWithContents(
//       async ({ windowId }) => {
...
//   Routes["/windows/#WINDOW_ID/focused"] = withWindow(
...
//       atob(dataUrl.substr("data:image/png;base64,".length)),
//       (c) => c.charCodeAt(0)
//     );
//   }),
//   async getattr() {
...
//           for (
//             let k = j === i ? startBracketIndex + 1 : 0;
//             k < jsLines[j].length;
//             k++
//           ) {
//             if (jsLines[j][k] === startBracket) {
//               counter++;
//             } else if (jsLines[j][k] === endBracket) {
//               counter--;
//             }
...
//       .map(([path, { usage, description, __isInfill, readdir }]) => {
//         if (__isInfill) {
//           return "";
//         }
//         let usages = usage ? (Array.isArray(usage) ? usage : [usage]) : [];
//         usages = usages.map((u) =>
//           u.replace("$0", path.substring(1) /* drop leading / */)
//         );
...
//           ${
//             usages.length > 0
//               ? `<dd><details><summary>Usage examples</summary>
//             <ul>
//               ${usages.map((u) => `<li>${u}</li>`).join("\n")}
//             </ul>
//           </details></dd>`
//               : '<dd style="background-color: #f99">No usage examples found!</dd>'
//           }
//           ${
//             lineRange
//               ? `<dd><details>
//               <summary>Source code (<a href="https://github.com/osnr/TabFS/blob/master/extension/background.js#L${
//                 lineRange[0] + 1
//               }-L${lineRange[1] + 1}">on GitHub</a>)</summary>
//               <pre><code>${
//                 jsLines.slice(lineRange[0], lineRange[1] + 1).join("\n")
//                 // FIXME: escape for HTML
//               }</code></pre>
//             </details></dd>`
//               : '<dd style="background-color: #f99">No source code found!</dd>'
//           }
//         `;
//       })
//       .join("\n") +
